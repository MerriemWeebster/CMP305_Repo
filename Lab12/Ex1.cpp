...
int main() {
    ...
    for (int n = DATASIZE/NBSAMPLES; n <= DATASIZE; n += DATASIZE/NBSAMPLES) {
        int avlHeight, avlMax = 0, avlMin = DATASIZE, avlAvg = 0,
            bstHeight, bstMax = 0, bstMin = DATASIZE, bstAvg = 0;

        for (int k = 1; k <= NBTREES; k++) {
            random_shuffle(data.begin(), data.begin() + n);
            if (SORTED > 0)
                partial_sort(data.begin(), data.begin()+n*SORTED/100, data.end());
            ///for (int i = 0; i < n; i++) cout << " " << data[i]; cout << endl;
            
            avl.makeEmpty(); bst.makeEmpty();
            for (int i = 0; i < n; i++) {
                avl.insert(data[i]); bst.insert(data[i]);
            }
            ///avl.pprint(); bst.pprint();
            
            avlHeight = avl.height(); avlAvg += avlHeight;
            avlMin = min(avlHeight,avlMin); avlMax = max(avlHeight,avlMax);
            bstHeight = bst.height(); bstAvg += bstHeight;
            bstMin = min(bstHeight,bstMin); bstMax = max(bstHeight,bstMax);
        }
